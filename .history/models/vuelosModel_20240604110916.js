...
async function obtenerPorId(Id) {
    const conexion = await obtenerConexion();
    try {
        const [results] = await conexion.query('SELECT * FROM Viajes WHERE id = ? ORDER BY id DESC', [Id]);
        return results;
    } catch (error) {
        console.error('Error al obtener el viaje por el id', error);
        throw error;
...
async function obtenerUbicacionPorid(id){
    const conexion = await obtenerConexion();
    try {
        const [iresults] = await conexion.query('SELECT * FROM ubicaciones WHERE id = ?', [1]);
